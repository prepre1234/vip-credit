...
# years>0.5的old_supplier,years<=0.5的new_supplier
old_supplier = all_supplier.query('years>0.5').reset_index(drop=True)
new_supplier = all_supplier.query('years<=0.5').reset_index(drop=True)
...
probability = bst.predict(pre_data_DMatrix)
...
new_add_vip_delReverse = new_add_vip[~new_add_vip['name'].isin(
    vip_0['name'])].reset_index(drop=True)
# # 获取new_add_vip_delReverse的信息
# new_add_vip_delReverse = pre_data_copy[pre_data_copy['name'].isin(
#     new_add_vip_delReverse['name'])].reset_index(drop=True)
# time = computeTime(new_add_vip_delReverse.iloc[:, -2:])
# time.columns = ['years']
# new_add_vip_delReverse = pd.concat(
#     [new_add_vip_delReverse.iloc[:, :2], time], axis=1)
# 新供应商&老供应商&不在数据库里的供应商
